...
const App = (() => {
    // Modal styles
    const modalStyle = `
    #employeeModal, #deleteModal { position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0); z-index:1000; }
    #employeeModal .modal-content, #deleteModal .modal-content { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); background:#fff; border-radius:8px; width:420px; max-width:90%; box-shadow:0 4px 24px rgba(0,0,0,0.15); }
    #employeeModal .modal-header, #deleteModal .modal-header { display:flex; justify-content:space-between; align-items:center; padding:16px 20px; border-bottom:1px solid #eee; }
    #employeeModal .modal-header h3, #deleteModal .modal-header h3 { margin:0; font-size:18px; }
    #employeeModal .modal-close, #deleteModal .modal-close { font-size:24px; cursor:pointer; color:#999; }
    #employeeModal .modal-close:hover, #deleteModal .modal-close:hover { color:#333; }
    #employeeModal .modal-body, #deleteModal .modal-body { padding:20px; }
    #employeeModal .modal-footer, #deleteModal .modal-footer { display:flex; justify-content:flex-end; gap:8px; padding:16px 20px; border-top:1px solid #eee; }
    ...
    #deleteModal .modal-content { width:340px; }
                            ...
                            ${data.map(e => `
                                <tr data-id="${e.id}">
                                    ...
                                    <td>
                                        ...
                                        <button class="btn-icon btn-icon-danger" onclick="App.openDeleteModal(${e.id}, '${escHtml(e.name)}')" title="删除"><i class="fas fa-trash"></i></button>
                                    </td>
                                </tr>
                            `).join('')}
                        </tbody>
    ...
    function openEmployeeModal(id) {
        ...
        if (id) {
            ...
            const emp = lastEmployees.find(e => e.id === id);
            if (!emp) return;
            document.getElementById('employeeId').value = id;
            document.getElementById('employeeName').value = emp.name;
            document.getElementById('employeeRole').value = emp.role;
            document.getElementById('employeeTeam').value = emp.team || '';
    ...
    async function saveEmployee() {
        ...
        if (!data.name.trim()) { toast('姓名不能为空', 'error'); return; }
        if (!data.role.trim()) { toast('角色不能为空', 'error'); return; }
